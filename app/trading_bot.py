...
class bot:
    def __init__(self, starting_money, buy_amount, selected_range=-1, filename_model=-1, filename_data=-1):
...
def auto_buy_sell_strategy(self, data, predictions):
        for i in range(0, len(predictions)):
                if(predictions[i] >= 0.9):
                    temp_buy = self.buy_amout * self.money
                    if self.money > temp_buy:
                        self.buy_times += 1
                        self.temp_usd_buys += data["close"].iloc[i]
                        self.temp_time_buys += data["close"].iloc[i]
        ...
        self.avg_btc_sell = round((self.avg_btc_sell / self.sell_times),2)
